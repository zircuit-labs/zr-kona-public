...
pub struct OverrideArgs {
    ...
    /// Manually specify the timestamp for the Tenrec fork, overriding the bundled setting.
    #[arg(long, env = "KONA_NODE_OVERRIDE_TENREC")]
    pub tenrec_override: Option<u64>,
    /// Manually specify the timestamp for the Fulu fork, overriding the bundled setting.
    #[arg(long, env = "KONA_NODE_OVERRIDE_FULU")]
    pub fulu_override: Option<u64>,
}
...
impl OverrideArgs {
    ...
    pub fn apply(&self, config: RollupConfig) -> RollupConfig {
        let hardforks = kona_genesis::HardForkConfig {
            ...
            tenrec_time: self.tenrec_override.map(Some).unwrap_or(config.hardforks.tenrec_time),
            fulu_time: self.fulu_override.map(Some).unwrap_or(config.hardforks.fulu_time),
        };
    ...
    fn test_apply_overrides() {
        ...
        assert_eq!(
            ...
            kona_genesis::HardForkConfig {
                ...
                tenrec_time: Some(1750000001),
                fulu_time: Some(1750000002),
            }
        );
    }
