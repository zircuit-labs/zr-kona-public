...
use kona_proof::boot::{
    AGREED_NONCE_KEY, AGREED_SENDER_ADDRESS_KEY, L1_CONFIG_KEY, L1_HEAD_KEY, L2_CHAIN_ID_KEY,
    L2_CLAIM_BLOCK_NUMBER_KEY, L2_CLAIM_KEY, L2_OUTPUT_ROOT_KEY, L2_ROLLUP_CONFIG_KEY,
    NONCE_CLAIM_KEY, SENDER_ADDRESS_CLAIM_KEY,
};
    ...
    fn get(&self, key: B256) -> Option<Vec<u8>> {
        ...
        match preimage_key.key_value() {
            ...
            L2_ROLLUP_CONFIG_KEY => {
                let rollup_config =
                    self.cfg.read_rollup_config().expect("unable to read rollup config");
                let serialized =
                    serde_json::to_vec(&rollup_config).expect("unable to serialize rollup config");
            ...
            AGREED_NONCE_KEY => Some(self.cfg.agreed_nonce.to_be_bytes().to_vec()),
            AGREED_SENDER_ADDRESS_KEY => Some(self.cfg.agreed_sender_address.to_vec()),
            NONCE_CLAIM_KEY => Some(self.cfg.claimed_nonce.to_be_bytes().to_vec()),
            SENDER_ADDRESS_CLAIM_KEY => Some(self.cfg.claimed_sender_address.to_vec()),
            _ => None,
        }
    }
