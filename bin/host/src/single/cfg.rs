...
pub struct SingleChainHost {
    ...
    /// Agreed Sender address.
    #[arg(long, env)]
    pub agreed_sender_address: Address,
    /// Claimed Sender nonce.
    #[arg(long, env)]
...
pub enum SingleChainHostError {
    ...
    /// An error when handling the client program.
    #[error("Client error: {0}")]
    FaultProofProgramError(#[from] kona_client::single::FaultProofProgramError),
    /// An IO error.
    #[error("IO error: {0}")]
...
impl SingleChainHost {
    ...
    async fn start_native(&self) -> Result<(), SingleChainHostError> {
        ...
        client_result.map_err(SingleChainHostError::from)
    }
...
mod test {
    ...
    fn test_flags() {
        ...
        let zero_address_str = &Address::ZERO.to_string();
        let default_flags = [
            "single",
            "--l1-head",
            zero_hash_str,
