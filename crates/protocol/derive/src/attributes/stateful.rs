    ...
    L2P: L2ChainProvider + Debug + Send,
{
    async fn prepare_payload_attributes(
        ...
        epoch: BlockNumHash,
        deposit_exclusions: Option<Bytes>,
        ...
        let sequence_number = if l2_parent.l1_origin.number != epoch.number {
            ...
            let deposits = derive_deposits(
                ...
                deposit_exclusions
                    .as_ref()
                    .map(|x| BitSet::try_from(x))
                    .transpose()
                    .map_err(|x| PipelineError::crit(x.into()))?,
            )
            .await
        ...
        if self.rollup_cfg.is_tenrec_active(next_l2_time)
            && !self.rollup_cfg.is_tenrec_active(l2_parent.block_info.timestamp)
        {
            upgrade_transactions.append(&mut Hardforks::TENREC.txs().collect());
        }
...
async fn derive_deposits(
    ...
    deposit_exclusions: Option<BitSet>,
    ...
    for r in receipts.iter() {
        ...
        for l in r.logs.iter() {
            ...
            deposit_index += 1;
            if deposit_exclusions.as_ref().map(|x| x.contains(deposit_index)).unwrap_or_default() {
                continue;
            }
...
mod tests {
    ...
    async fn test_derive_deposits_empty() {
        ...
        let result = derive_deposits(B256::default(), &receipts, deposit_contract, None).await;
    ...
    #[tokio::test]
    async fn test_derive_deposits_exclusions_filtered_out() {
        let deposit_contract = address!("1111111111111111111111111111111111111111");
        let mut invalid = generate_valid_receipt();
        invalid.logs[0].data = LogData::new_unchecked(vec![], Bytes::default());
        let receipts = vec![generate_valid_receipt(), generate_valid_receipt(), invalid];
        let exclusions =
            BitSet::try_from(&Bytes::from(0x00000000000000020000000000000002_u128.to_be_bytes()))
                .unwrap();
        let result =
            derive_deposits(B256::default(), &receipts, deposit_contract, Some(exclusions)).await;
        assert_eq!(result.unwrap().len(), 4);
    }
