...
impl<F: ChainProvider> PollingTraversal<F> {
    ...
    pub fn new(data_source: F, cfg: Arc<RollupConfig>, l1_blocks: Option<BTreeSet<u64>>) -> Self {
        Self {
...
impl<F: ChainProvider + Send> OriginAdvancer for PollingTraversal<F> {
    ...
    async fn advance_origin(&mut self) -> PipelineResult<()> {
        ...
        if self.l1_blocks.as_ref().map(|x| !x.contains(&next_l1_origin.number)).unwrap_or_default()
        {
