    ...
    P: L1RetrievalProvider + OriginAdvancer + OriginProvider + SignalReceiver,
{
    ...
    /// A set containing the L1 blocks numbers with DA transactions.
    pub l1_blocks: Option<BTreeSet<u64>>,
}
    ...
    P: L1RetrievalProvider + OriginAdvancer + OriginProvider + SignalReceiver,
{
    ...
    ///
    /// [`PollingTraversal`]: crate::PollingTraversal
    pub const fn new(prev: P, provider: DAP, l1_blocks: Option<BTreeSet<u64>>) -> Self {
        Self { prev, provider, next: None, l1_blocks }
    }
}
    ...
    P: L1RetrievalProvider + OriginAdvancer + OriginProvider + SignalReceiver + Send,
{
    ...
    async fn next_data(&mut self) -> PipelineResult<Self::Item> {
        ...
        if self.l1_blocks.as_ref().map(|x| !x.contains(&next.number)).unwrap_or_default() {
            self.next = None;
            self.provider.clear();
            return Err(PipelineError::temp(PipelineError::Eof));
        }
...
mod tests {
    ...
    async fn test_l1_retrieval_flush_channel() {
        ...
        let mut retrieval = L1Retrieval::new(traversal, dap, None);
    ...
    async fn test_l1_retrieval_origin() {
        let traversal = TraversalTestHelper::new_populated();
        let dap = TestDAP { results: vec![] };
        let retrieval = L1Retrieval::new(traversal, dap, None);
