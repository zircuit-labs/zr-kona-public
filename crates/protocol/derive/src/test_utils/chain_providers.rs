...
impl ChainProvider for TestChainProvider {
    ...
    /// Returns a [BTreeSet] containing the L1 block numbers with DA transactions.
    async fn block_numbers(&mut self, _l1_origin: u64) -> Result<BTreeSet<u64>, Self::Error> {
        let block_numbers: BTreeSet<u64> = self.blocks.iter().map(|(number, _)| *number).collect();
        Ok(block_numbers)
    }
}
