    ...
    B: BlobProvider + Send,
{
    ...
    pub open: bool,
    /// Initialization Nonce, None if skipping checks.
    pub nonce_check_start: Option<(Address, u64)>,
    /// Tracking nonce.
    pub tracking_nonce: Option<(Address, u64)>,
}
    ...
    B: BlobProvider + Send,
{
    ...
    pub const fn new(
        ...
        nonce_check_start: Option<(Address, u64)>,
    ) -> Self {
        Self {
            chain_provider,
            blob_fetcher,
    ...
    fn extract_blob_data(
        ...
        for tx in txs {
            let (tx_kind, calldata, blob_hashes) = match &tx {
                ...
                TxEnvelope::Eip7702(tx) => (tx.tx().to(), tx.tx().input.clone(), None),
            };
            ...
            if let Some(nonce_tracker) = &mut self.tracking_nonce {
                if self.nonce_check_start.is_some() && tx_nonce != nonce_tracker.1 {
                    return Err(BlobProviderError::NotSequentialNonce(
                        nonce_tracker.1 + 1,
                        tx_nonce,
                    ));
                }

                nonce_tracker.0 = batcher_address;
                nonce_tracker.1 = tx_nonce + 1;
            } else {
                if let Some(nonce_check_start) = self.nonce_check_start {
                    if nonce_check_start.0 != batcher_address {
                        return Err(BlobProviderError::AgreedSenderAddressMissmatch(
                            nonce_check_start.0,
                            batcher_address,
                        ));
                    }

                    if tx_nonce > nonce_check_start.1 {
                        return Err(BlobProviderError::NonceTooHigh(nonce_check_start.1, tx_nonce));
                    }
                }

                let initial_nonce =
                    self.nonce_check_start.map(|v| v.1).unwrap_or(tx_nonce).min(tx_nonce);
                self.tracking_nonce = Some((batcher_address, initial_nonce + 1));
            }
