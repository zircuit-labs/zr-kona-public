...
use alloc::{boxed::Box, fmt::Debug, sync::Arc};
    ...
    B: BlobProvider + Send + Clone,
{
    ...
    /// The blob source.
    pub blob_source: Arc<Mutex<BlobSource<C, B>>>,
    /// The calldata source.
    ...
    B: BlobProvider + Send + Clone + Debug,
{
    ...
    pub fn new(
        ...
        Self {
            ...
            blob_source: Arc::new(Mutex::new(blob_source)),
            calldata_source,
        }
    }
    ...
    pub fn new_from_parts(
        ...
        blobs: B,
        cfg: &RollupConfig,
        nonce: Option<(Address, u64)>,
    ) -> Self {
        Self {
            ecotone_timestamp: cfg.hardforks.ecotone_time,
            blob_source: Arc::new(Mutex::new(BlobSource::new(
                provider.clone(),
                blobs,
    ...
    B: BlobProvider + Send + Sync + Clone + Debug,
{
    ...
    fn final_nonce(&self) -> Option<(Address, u64)> {
        self.blob_source.lock().tracking_nonce
    }
    ...
    async fn next(
        ...
        if ecotone_enabled {
            self.blob_source.lock().next(block_ref, batcher_address).await
        } else {
...
mod tests {
    ...
    async fn test_clear_ethereum_data_source() {
        ...
        data_source.clear();
        assert!(data_source.blob_source.lock().data.is_empty());
        assert!(!data_source.blob_source.lock().open);
    ...
    async fn test_open_ethereum_calldata_source_pre_ecotone() {
        ...
        let mut data_source = EthereumDataSource::new_from_parts(chain, blob, &cfg, None);
