...
// TODO: re-enable once all rollup config fields are supported (otherwise the stuck on pre-image requests)
...
impl RollupConfig {
    ...
    pub fn is_holocene_active(&self, timestamp: u64) -> bool {
        self.hardforks.holocene_time.is_some_and(|t| timestamp >= t)
            || self.is_tenrec_active(timestamp)
    ...
    /// Returns true if Tenrec is active at the given timestamp.
    pub fn is_tenrec_active(&self, timestamp: u64) -> bool {
        self.hardforks.tenrec_time.is_some_and(|t| timestamp >= t) || self.is_fulu_active(timestamp)
    }

    /// Returns true if Fulu is active at the given timestamp.
    pub fn is_fulu_active(&self, timestamp: u64) -> bool {
        self.hardforks.fulu_time.is_some_and(|t| timestamp >= t)
    }

    /// Returns true if the timestamp marks the first Tenrec block.
    pub fn is_first_tenrec_block(&self, timestamp: u64) -> bool {
        self.is_tenrec_active(timestamp)
            && !self.is_tenrec_active(timestamp.saturating_sub(self.block_time))
    }
...
mod tests {
    ...
    fn test_revm_spec_id() {
        ...
        config.hardforks.tenrec_time = Some(60);
    ...
    fn test_is_first_fork_block() {
        ...
        // Isthmus
        assert!(!cfg.is_first_isthmus_block(43));
        assert!(cfg.is_first_isthmus_block(45));
        assert!(!cfg.is_first_isthmus_block(47));
    ...
    // We allow unknown fields for forward compatibility.
    // #[test]
