...
pub struct HardForkConfig {
    ...
    /// `tenrec_time` sets the activation time for the Tenrec network upgrade.
    /// Active if `tenrec_time` != None && L2 block timestamp >= Some(tenrec_time), inactive
    /// otherwise.
    #[cfg_attr(feature = "serde", serde(skip_serializing_if = "Option::is_none"))]
    pub tenrec_time: Option<u64>,
    /// `fulu_time` sets the activation time for the Fulu network upgrade.
    /// Active if `fulu_time` != None && L2 block timestamp >= Some(fulu_time), inactive
    /// otherwise.
    #[cfg_attr(feature = "serde", serde(skip_serializing_if = "Option::is_none"))]
...
mod tests {
    ...
    fn test_hardforks_iter() {
        ...
        assert_eq!(iter.next(), Some(("Tenrec", Some(12))));
        assert_eq!(iter.next(), Some(("Fulu", Some(13))));
