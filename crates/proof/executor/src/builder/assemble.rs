    ...
    Evm: EvmFactory,
{
    ...
    pub(crate) fn seal_block(
        ...
        let withdrawals_root = if self.config.is_isthmus_active(timestamp) {
            Some(self.fetch_withdrawal_root(block_env.number.saturating_to::<u64>(), timestamp)?)
    ...
    pub fn compute_output_root(&mut self) -> ExecutorResult<B256> {
        ...
        let storage_root =
            self.fetch_withdrawal_root(parent_header.number, parent_header.timestamp)?;
        let parent_header = self.trie_db.parent_block_header();
    ...
    /// Fetches the withdrawal root for the current block.
    /// The withdrawal root represents the Merkle root of all pending withdrawals from L2 to L1.
    fn fetch_withdrawal_root(
        &mut self,
        block_number: u64,
        timestamp: u64,
    ) -> Result<B256, TrieDBError> {
        if self.config.is_tenrec_active(timestamp) {
            let withdrawal_root = self.trie_db.fetch_storage(
                Predeploys::L2_TO_L1_MESSAGE_PASSER,
                Predeploys::WITHDRAWAL_ROOT_STORAGE_SLOT.into(),
            )?;
            Ok(B256::from(withdrawal_root))
        } else {
            // Fetches the L2 to L1 message passer account storage root from the cache or underlying trie.
