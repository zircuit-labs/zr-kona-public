...
pub struct OracleL1ChainProvider<T: CommsClient> {
    ...
    pub oracle: Arc<T>,
    /// The header cache used to avoid repeating fetching/decoding
    pub header_cache: alloc::collections::BTreeMap<u64, Header>,
}
...
impl<T: CommsClient> OracleL1ChainProvider<T> {
    ...
    pub const fn new(l1_head: B256, oracle: Arc<T>) -> Self {
        Self { l1_head, oracle, header_cache: alloc::collections::BTreeMap::new() }
    }
}
...
impl<T: CommsClient + Sync + Send> ChainProvider for OracleL1ChainProvider<T> {
    ...
    async fn header_by_hash(&mut self, hash: B256) -> Result<Header, Self::Error> {
        ...
        self.header_cache.entry(header.number).or_insert_with(|| header.clone());
    ...
    async fn block_info_by_number(&mut self, block_number: u64) -> Result<BlockInfo, Self::Error> {
        #[cfg(target_os = "zkvm")]
        println!("cycle-tracker-report-start: block-info");

        // Fetch the starting block header.

        let mut header = if let Some(value) = self.header_cache.get(&block_number) {
            value.clone()
        } else {
            self.header_by_hash(self.l1_head).await?
        };
        ...
        #[cfg(target_os = "zkvm")]
        println!("cycle-tracker-report-end: block-info");
        Ok(BlockInfo {
    ...
    async fn receipts_by_hash(&mut self, hash: B256) -> Result<Vec<Receipt>, Self::Error> {
        #[cfg(target_os = "zkvm")]
        println!("cycle-tracker-report-start: receipt");
        ...
        #[cfg(target_os = "zkvm")]
        println!("cycle-tracker-report-end: receipt");
    ...
    async fn block_info_and_transactions_by_hash(
        ...
        #[cfg(target_os = "zkvm")]
        println!("cycle-tracker-report-start: transactions");
        ...
        #[cfg(target_os = "zkvm")]
        println!("cycle-tracker-report-end: transactions");

        Ok((block_info, transactions))
    }

    async fn block_numbers(&mut self, l1_origin: u64) -> Result<BTreeSet<u64>, Self::Error> {
        HintType::L1Blocks
            .with_data(&[&l1_origin.to_be_bytes()])
        ...
        Ok(BTreeSet::from_iter(
            header_rlp.chunks(size_of::<u64>()).map(|x| u64::from_be_bytes(x.try_into().unwrap())),
        ))
    }
}
